  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (skillElement) {
          const skillTitle = skillElement.querySelector('.skill-title')?.textContent || '';
...
function initScrollAnimations() {
  ...
  document.querySelectorAll('.about-card, .skill, .leetcode-item, .contact-card').forEach((card, index) => {
    if (index < 5) {
...
function initCardHoverEffects() {
  const cards = document.querySelectorAll('.about-card, .skill, .project-card, .contact-card, .resume-card');
...
function initAppleStyleScrolling() {
  ...
  const revealElements = document.querySelectorAll('.about-card, .skill, .project-card, .contact-card, .leetcode-item, .resume-card');
  ...
  window.addEventListener('scroll', throttle(() => {
    ...
    sections.forEach((section, index) => {
      ...
      const progress = Math.max(0, Math.min(1, (window.innerHeight - sectionTop) / window.innerHeight));
